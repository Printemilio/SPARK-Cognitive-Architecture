...
static double kd_point_dist_sq(const BubbleShadowMemory *m, size_t idx, const double target[3], double energy_weight, double *out_spatial) {
    const Bubble *b = &m->bubbles[idx];
    double dx = target[0] - b->s[0];
    double dy = target[1] - b->s[1];
    
    // Mahalanobis-ish distance using the 2x2 covariance S
    // S = [S0 S1; S2 S3]
    // det = S0*S3 - S1*S2
    double det = b->S[0] * b->S[3] - b->S[1] * b->S[2];
    double d_spatial_sq;
    if (fabs(det) > 1e-9) {
        double invDet = 1.0 / det;
        // invS = (1/det) * [S3 -S1; -S2 S0]
        double d_mahal = (b->S[3] * dx * dx - (b->S[1] + b->S[2]) * dx * dy + b->S[0] * dy * dy) * invDet;
        d_spatial_sq = d_mahal;
    } else {
        d_spatial_sq = dx * dx + dy * dy;
    }
    
    double de = target[2] - b->a;
    if (out_spatial) *out_spatial = sqrt(dx * dx + dy * dy);
    de *= energy_weight;
    return d_spatial_sq + de * de;
}
...
void bubble_memory_store(BubbleShadowMemory *m, const double *x, size_t dim, double emotion) {
    ...
    }
    {
        ...
        target->dr = 0.9 * target->dr + 0.1 * da;
    }
    target->n += 1;
    {
        // Update covariance S based on the movement in shadow space (s0, s1)
        double ds0 = sx[0] - prev_s0;
        double ds1 = sx[1] - prev_s1;
        // incremental covariance update (simplified)
        double beta = alpha; 
        target->S[0] = (1.0 - beta) * target->S[0] + beta * (ds0 * ds0);
        target->S[1] = (1.0 - beta) * target->S[1] + beta * (ds0 * ds1);
        target->S[2] = (1.0 - beta) * target->S[2] + beta * (ds1 * ds0);
        target->S[3] = (1.0 - beta) * target->S[3] + beta * (ds1 * ds1);
...
double bubble_memory_project(BubbleShadowMemory *m, const double *x, size_t dim) {
    ...
    if (idx != (size_t)-1) {
        ...
        // Use the same Mahalanobis-ish distance for familiarity if available
        double target[3] = {sx[0], sx[1], 0.0};
        double metric_dist_sq = kd_point_dist_sq(m, idx, target, 0.0, NULL);
        double dist_score = exp(-metric_dist_sq); // Higher similarity = closer to 1.0

        fam = dist_score * fmax(0.0, dir);
    }
    m->recalls++;
    return fam;
}
...
void bubble_memory_feedback(BubbleShadowMemory *m, const double *x, size_t dim, double reward) {
    ...
    double best_dist;
    size_t idx = bubble_nearest(m, sx, 0.0, 0.0, &best_dist);
    if (idx == (size_t)-1 || best_dist > m->match_tau) return;
    Bubble *b = &m->bubbles[idx];
    double dir = cosine_similarity_vec(vx, b->c, dim);
    if (dir < m->dir_tau) return;
    b->dr = 0.9 * b->dr + 0.1 * reward;
}
...
void bubble_memory_merge_close(BubbleShadowMemory *m, double shadow_thresh, double dir_thresh) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        double acc_dr = bi->dr * (double)bi->n;
...
void memory_shim_feedback(MemoryShim *shim, const double *x, size_t dim, double reward) {
    if (!shim) return;
    bubble_memory_feedback(shim->mem, x, dim, reward);
}
...
// ---------- Deferred reward ----------
static size_t deferred_event_index(const DeferredRewardModule *m, size_t order) {
    if (!m || m->cap == 0 || m->len == 0) return 0;
    size_t oldest = (m->head + m->cap - m->len) % m->cap;
    return (oldest + order) % m->cap;
}
...
DeferredRewardModule *deferred_reward_create(size_t history_cap, double temporal_tau, double connectivity_bonus, double credit_persistence) {
    DeferredRewardModule *m = calloc(1, sizeof(DeferredRewardModule));
    if (!m) return NULL;
    m->cap = history_cap > 0 ? history_cap : 512;
    m->events = calloc(m->cap, sizeof(DeferredActivationEvent));
    if (!m->events) {
        free(m);
        return NULL;
    }
    m->temporal_tau = temporal_tau > 1e-6 ? temporal_tau : 20.0;
    m->goal_floor = 0.2;
    m->connectivity_bonus = connectivity_bonus;
    m->credit_persistence = spark_clip(credit_persistence, 0.0, 1.0);
    return m;
}
...
void deferred_reward_record_batch(DeferredRewardModule *m, const size_t *active_ids, size_t count, size_t tick) {
    if (!m || !m->events || m->cap == 0) return;
    m->latest_tick = tick;
    if (!active_ids || count == 0) return;
    for (size_t i = 0; i < count; ++i) {
        DeferredActivationEvent ev;
        ev.node_id = active_ids[i];
        ev.tick = tick;
        m->events[m->head] = ev;
...
void deferred_reward_apply(DeferredRewardModule *m, CognitiveGraph *graph, double reward, const double *final_stimulus, size_t dim) {
    if (!m || !graph || graph->num_nodes == 0) return;
    if (!isfinite(reward) || fabs(reward) < 1e-12) return;
    if (!m->events || m->len == 0 || m->cap == 0) return;

    size_t n_nodes = graph->num_nodes;
    double *node_raw = calloc(n_nodes, sizeof(double));
    double *middle_counts = calloc(n_nodes, sizeof(double));
    double *goal = calloc(graph->state_dim, sizeof(double));
    if (!node_raw || !middle_counts || !goal) {
        free(node_raw);
        free(middle_counts);
        free(goal);
        return;
    ...
    int has_goal = 0;
    if (final_stimulus && graph->state_dim > 0) {
        size_t copy_dim = dim < graph->state_dim ? dim : graph->state_dim;
        memcpy(goal, final_stimulus, copy_dim * sizeof(double));
        if (spark_norm(goal, graph->state_dim) > 1e-12) {
            spark_normalize(goal, graph->state_dim);
            has_goal = 1;
        }
    }
    ...
    for (size_t i = 0; i < n_nodes; ++i) {
        ...
        double degree = (double)(n->outgoing.count + n->incoming.count);
        if (degree > max_degree) max_degree = degree;
    }

    for (size_t pos = 1; pos + 1 < m->len; ++pos) {
        size_t idx = deferred_event_index(m, pos);
        size_t node_id = m->events[idx].node_id;
        if (node_id < n_nodes) middle_counts[node_id] += 1.0;
    }

    double max_middle = 1.0;
    for (size_t i = 0; i < n_nodes; ++i) {
        if (middle_counts[i] > max_middle) max_middle = middle_counts[i];
    }

    double total_raw = 0.0;
    for (size_t pos = 0; pos < m->len; ++pos) {
        size_t idx = deferred_event_index(m, pos);
        DeferredActivationEvent ev = m->events[idx];
        if (ev.node_id >= n_nodes) continue;
        Node *node = &graph->nodes[ev.node_id];

        size_t dt = (m->latest_tick > ev.tick) ? (m->latest_tick - ev.tick) : 0;
        double temporal = exp(-(double)dt / m->temporal_tau);
        double goal_factor = 1.0;
        if (has_goal) {
            double sim = spark_cosine_similarity(node->state, goal, graph->state_dim);
            goal_factor = m->goal_floor + (1.0 - m->goal_floor) * fmax(0.0, sim);
        }
        double degree_norm = (double)(node->outgoing.count + node->incoming.count) / max_degree;
        double path_middle_norm = middle_counts[ev.node_id] / max_middle;
        double connectivity = 1.0 + m->connectivity_bonus * (0.6 * degree_norm + 0.4 * path_middle_norm);
        double raw = temporal * goal_factor * connectivity;
        node_raw[ev.node_id] += raw;
        total_raw += raw;
    }
    ...
    for (size_t i = 0; i < n_nodes; ++i) {
        if (node_raw[i] <= 0.0) continue;
        Node *node = &graph->nodes[i];
        double contrib = reward * (node_raw[i] / total_raw);
        node->deferred_credit = spark_clip(node->deferred_credit * m->credit_persistence + contrib, -4.0, 4.0);
    }
...
static void deferred_reward_trace_hook(const size_t *active_ids, size_t count, size_t tick, void *ctx) {
    DeferredRewardModule *m = (DeferredRewardModule *)ctx;
    deferred_reward_record_batch(m, active_ids, count, tick);
}
...
void affect_register_feedback(AffectModule *a, double reward, const double *state, size_t dim) {
    ...
    if (a->deferred_reward && a->graph && state && dim > 0) {
        deferred_reward_apply(a->deferred_reward, a->graph, reward, state, dim);
    }
    ...
    if (a->global_memory) {
        memory_shim_feedback(a->global_memory, state, dim, reward);
    }
}
...
void cg_plasticity_rule(Node *node, Node **neighbors, double *weights, size_t neighbor_count, CognitiveGraph *graph) {
    ...
    for (size_t i = 0; i < neighbor_count; ++i) {
        Node *nb = neighbors[i];
        // FIX: nb->history also records (t-1). We want current nb->state (t).
        double dot = 0.0;
...
SparkSystem *build_spark_system(int seed) {
    ...
    size_t deferred_cap = 2048;
    char *env_deferred_cap = getenv("SPARK_DEFERRED_CAP");
    if (env_deferred_cap) {
        long v = strtol(env_deferred_cap, NULL, 10);
        if (v > 16) deferred_cap = (size_t)v;
    }
    double deferred_tau = 20.0;
    char *env_deferred_tau = getenv("SPARK_DEFERRED_TAU");
    if (env_deferred_tau) {
        double v = strtod(env_deferred_tau, NULL);
        if (v > 1e-6) deferred_tau = v;
    }
    double deferred_connectivity_bonus = 0.35;
    char *env_deferred_bonus = getenv("SPARK_DEFERRED_CONNECT_BONUS");
    if (env_deferred_bonus) {
        deferred_connectivity_bonus = strtod(env_deferred_bonus, NULL);
    }
    double deferred_persistence = 0.97;
    char *env_deferred_persistence = getenv("SPARK_DEFERRED_PERSISTENCE");
    if (env_deferred_persistence) {
        deferred_persistence = strtod(env_deferred_persistence, NULL);
    }
    DeferredRewardModule *deferred = deferred_reward_create(
        deferred_cap, deferred_tau, deferred_connectivity_bonus, deferred_persistence);
    ...
    if (deferred) {
        cg_set_activation_trace(graph, deferred_reward_trace_hook, deferred);
    }
    affect->graph = graph;
    affect->deferred_reward = deferred;
...
void spark_system_free(SparkSystem *sys) {
    ...
    deferred_reward_free(sys->deferred_reward);
