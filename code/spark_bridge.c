...
#define SPARK_CORTEX_HORIZON 10
#define SPARK_CORTEX_PANIC 0.7
#define SPARK_CORTEX_POS_THRESH 0.5
#define SPARK_CORTEX_NEG_THRESH -0.2
#define SPARK_CORTEX_POS_GAIN 0.1
#define SPARK_CORTEX_NEG_NOISE 0.2

static void bridge_set_last_state(SparkBridge *bridge, const double *state, size_t dim) {
    if (!bridge || !state || dim == 0) return;
    if (!bridge->last_state || bridge->last_state_len != dim) return;
    memcpy(bridge->last_state, state, dim * sizeof(double));
    bridge->has_last_state = 1;
}
...
static int ensure_visual_capacity(SparkBridge *bridge, size_t need) {
    ...
    if (bridge->visual_ids_cap != 0) return 0;
    bridge->visual_ids = calloc(need, sizeof(int));
    if (!bridge->visual_ids) return 0;
    bridge->visual_ids_cap = need;
    return 1;
}
...
SparkBridge *spark_bridge_create(SparkSystem *sys, size_t frame_width, size_t frame_height, size_t visual_offset) {
    ...
    b->has_prev = 0;
    b->cortex = NULL;
    b->last_state_len = sys->graph->state_dim;
    b->last_state = calloc(b->last_state_len, sizeof(double));
    b->has_last_state = 0;
    b->scratch_vec_len = sys->graph->state_dim;
    b->scratch_vec = calloc(b->scratch_vec_len, sizeof(double));
    if (sys->global_memory && sys->affect_module) {
        b->cortex = spark_cortex_create(sys->global_memory->mem, sys->affect_module, sys->graph->state_dim);
    }
    b->motor_nas = NULL;
    return b;
}
...
void spark_bridge_free(SparkBridge *bridge) {
    ...
    spark_cortex_free(bridge->cortex);
    free(bridge->visual_ids);
    free(bridge->prev_frame);
    free(bridge->last_state);
    free(bridge->scratch_vec);
    free(bridge);
}
...
void spark_bridge_reset_history(SparkBridge *bridge) {
    ...
    bridge->has_last_state = 0;
    if (bridge->prev_frame) spark_zero(bridge->prev_frame, bridge->prev_frame_len);
    if (bridge->cortex) spark_cortex_reset_history(bridge->cortex);
}
...
void spark_bridge_inject_frame(SparkBridge *bridge, const double *frame) {
    ...
    CognitiveGraph *g = bridge->sys->graph;
    size_t dim = g->state_dim;
    if (!bridge->scratch_vec || bridge->scratch_vec_len < dim) return;
    ...
    for (size_t ry = 0; ry < bridge->retina_h; ++ry) {
        ...
        for (size_t rx = 0; rx < bridge->retina_w; ++rx) {
            ...
            bridge_set_last_state(bridge, vec, dim);
            patch_idx++;
            if (patch_idx >= spark_bridge_patch_count(bridge)) break;
        }
        if (patch_idx >= spark_bridge_patch_count(bridge)) break;
    }
    if (!bridge->prev_frame || bridge->prev_frame_len != bridge->frame_width * bridge->frame_height) return;
...
void spark_bridge_resolve_outputs(SparkBridge *bridge, double *out_signals, size_t signal_len) {
    ...
    double pain = 0.0;
    if (bridge->sys->affect_module && bridge->sys->affect_module->affect_buffer && bridge->sys->affect_module->buf_len > 0) {
        double affect_now = bridge->sys->affect_module->affect_buffer[0];
        pain = fmax(0.0, -affect_now);
    }

    if (pain <= SPARK_CORTEX_PANIC && bridge->cortex && bridge->has_last_state) {
        double score = spark_cortex_predict_trajectory(bridge->cortex, bridge->last_state, SPARK_CORTEX_HORIZON);
        for (size_t m = 0; m < nas->cfg.num_channels; ++m) {
            double v = out_signals[m];
            if (score > SPARK_CORTEX_POS_THRESH) {
                double gain = 1.0 + fmin(0.5, score * SPARK_CORTEX_POS_GAIN);
                v = spark_clip(v * gain, -1.0, 1.0);
            } else if (score < SPARK_CORTEX_NEG_THRESH) {
                double noise = (spark_rand_uniform() * 2.0 - 1.0) * SPARK_CORTEX_NEG_NOISE;
                v = spark_clip((-v * 0.7) + noise, -1.0, 1.0);
            }
            out_signals[m] = v;
        }
    }
