...
#define SPARK_GRAPH_POOL_DEFAULT 4096
#define SPARK_WEIGHT_HOMEOSTASIS_DEFAULT 1.0
...
static size_t graph_default_pool_cap(size_t min_need) {
    static size_t cached = 0;
    if (cached == 0) {
        cached = SPARK_GRAPH_POOL_DEFAULT;
        const char *env = getenv("SPARK_GRAPH_POOL_CAP");
        if (env && env[0]) {
            long v = strtol(env, NULL, 10);
            if (v > 0) cached = (size_t)v;
        }
    }
    return cached > min_need ? cached : min_need;
}

static double weight_homeostasis_target(void) {
    static double cached = -1.0;
    if (cached >= 0.0) return cached;
    cached = SPARK_WEIGHT_HOMEOSTASIS_DEFAULT;
    const char *env = getenv("SPARK_WEIGHT_HOMEOSTASIS");
    if (env && env[0]) {
...
static float cg_norm_sq_f32(const double *v, size_t n) {
    if (!v || n == 0) return 0.0f;
#if defined(__AVX__)
    __m256 acc = _mm256_setzero_ps();
    size_t i = 0;
    for (; i + 8 <= n; i += 8) {
        __m256 vf = _mm256_set_ps((float)v[i + 7], (float)v[i + 6], (float)v[i + 5], (float)v[i + 4],
                                  (float)v[i + 3], (float)v[i + 2], (float)v[i + 1], (float)v[i]);
        acc = _mm256_add_ps(acc, _mm256_mul_ps(vf, vf));
    }
    float lanes[8];
...
static double node_credit_gain(const Node *n) {
    if (!n) return 1.0;
    double shaped = tanh(n->deferred_credit);
    return spark_clip(1.0 + 0.4 * shaped, 0.6, 1.4);
}
...
static void graph_reserve(CognitiveGraph *g, size_t need) {
    ...
    if (g->capacity == 0) {
        size_t cap = g->max_nodes ? g->max_nodes : graph_default_pool_cap(need);
        if (cap < need) cap = need;
        g->nodes = xcalloc(cap, sizeof(Node));
        g->active_current = xcalloc(cap, sizeof(size_t));
        g->active_next = xcalloc(cap, sizeof(size_t));
        g->scheduled = xcalloc(cap, sizeof(char));
        g->touched_nodes = xcalloc(cap, sizeof(size_t));
        g->touched_flag = xcalloc(cap, sizeof(char));
        g->nbr_nodes = xcalloc(cap, sizeof(Node *));
        g->nbr_weights = xcalloc(cap, sizeof(double));
        g->capacity = cap;
        g->nbr_cap = cap;
    }
}
...
static Node *graph_new_node(CognitiveGraph *g, const double *state_vec) {
    ...
    if (g->num_nodes >= g->capacity) return NULL;
...
static void update_node_activation(CognitiveGraph *g, Node *n) {
    double gain = node_credit_gain(n);
    double base_energy_sq = (double)cg_norm_sq_f32(n->state, n->dim);
    double energy_sq = base_energy_sq * gain * gain;
    n->energy = sqrt(fmax(energy_sq, 0.0));
    double threshold_sq = g->activation_threshold * g->activation_threshold;
    if (energy_sq >= threshold_sq) {
        n->sleeping = 0;
        n->inactivity_streak = 0;
    } else {
...
static void normalize_outgoing_homeostasis(CognitiveGraph *g, size_t src_id) {
    if (!g || src_id >= g->num_nodes) return;
    Node *src = &g->nodes[src_id];
    if (src->outgoing.count == 0) return;

    double sumsq = 0.0;
    for (size_t i = 0; i < src->outgoing.count; ++i) {
        double w = src->outgoing.synapses[i].weight;
        sumsq += w * w;
    }
    if (sumsq < 1e-16) return;

    double target = weight_homeostasis_target();
    double scale = target / sqrt(sumsq);
    for (size_t i = 0; i < src->outgoing.count; ++i) {
        src->outgoing.synapses[i].weight *= scale;
    }

    for (size_t i = 0; i < src->outgoing.count; ++i) {
        int dst_idx = src->outgoing.ids[i];
        if (dst_idx < 0 || (size_t)dst_idx >= g->num_nodes) continue;
        Node *dst = &g->nodes[(size_t)dst_idx];
        ssize_t in_idx = syn_list_index(&dst->incoming, (int)src_id);
        if (in_idx >= 0) {
            dst->incoming.synapses[(size_t)in_idx].weight = src->outgoing.synapses[i].weight;
        }
    }
}
...
CognitiveGraph *cg_create(size_t num_nodes, size_t state_dim, double activation_threshold, int sleep_patience, size_t num_segments, double segment_threshold, double segment_activation_ratio) {
    ...
    if (g->max_nodes == 0) {
        g->max_nodes = graph_default_pool_cap(16);
    }
    ...
    g->hnsw_max_level = 0;
    g->hnsw_M = 8;
    g->activation_trace_fn = NULL;
    g->activation_trace_ctx = NULL;
    g->propagation_tick = 0;
...
void cg_connect(CognitiveGraph *g, size_t src, size_t dst, double weight) {
    if (!g) return;
    if (!graph_ensure_node(g, src) || !graph_ensure_node(g, dst)) return;
    Node *src_node = &g->nodes[src];
    Synapse *syn = syn_list_get_or_add(&src_node->outgoing, (int)dst);
    syn->weight = weight;
    normalize_outgoing_homeostasis(g, src);
...
void cg_set_activation_trace(CognitiveGraph *g, CGActivationTraceFn fn, void *ctx) {
    if (!g) return;
    g->activation_trace_fn = fn;
    g->activation_trace_ctx = ctx;
}
...
void cg_propagate(CognitiveGraph *g, RuleFn *rules, size_t rule_count) {
    if (!g) return;
    if (g->activation_trace_fn) {
        g->activation_trace_fn(g->active_current, g->active_current_len, g->propagation_tick, g->activation_trace_ctx);
    }
    g->propagation_tick++;
    ...
    for (size_t idx = 0; idx < g->active_current_len; ++idx) {
        ...
        update_node_activation(g, src);
        double src_energy = src->energy;
